...
double_tensor MLPClassifier::predict(
    ...
    for(auto batch: *pLoader){
        double_tensor X = batch.getData(); // Get input data from the batch
        double_tensor Y = this->predict(X, false);
...
double_tensor MLPClassifier::evaluate(DataLoader<double, double>* pLoader){
    ...
    for(auto batch: *pLoader){
        double_tensor X = batch.getData(); // Get input data from the batch
        double_tensor t = batch.getLabel(); // Get true labels from the batch
        
        // Get predictions for the batch
        double_tensor Y = this->forward(X); // Get predictions without making decisions
        
        // Compute true and predicted labels
        ulong_tensor y_true = xt::argmax(t, 1); // True labels (assuming t is one-hot encoded)
        ulong_tensor y_pred = xt::argmax(Y, 1); // Predicted labels
        ...
        // cout << fmt::format("Evaluating Batch {}/{}: {} samples processed\n", 
...
void MLPClassifier::backward(){
    ...
    for (it = m_layers.bbegin(); it != m_layers.bend(); it++) 
    {
        ILayer* layer = *it;
