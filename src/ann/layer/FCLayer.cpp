...
xt::xarray<double> FCLayer::forward(xt::xarray<double> X) {
    //YOUR CODE IS HERE
    m_aCached_X = X;
    // return xt::linalg::dot(xt::transpose(this->m_aWeights), X) + m_aBias; // Shape: (Nout, Nbatch)
    xt::xarray<double> output = xt::linalg::dot(X, xt::transpose(this->m_aWeights));
    if (m_bUse_Bias)
        output += xt::expand_dims(m_aBias, 0);
    return output;

}
// xt::xarray<double> FCLayer::backward(xt::xarray<double> DY) {
//     //YOUR CODE IS HERE
//     xt::xarray<double> dX = xt::linalg::dot(xt::transpose(this->m_aWeights), DY); // Shape: (Nin, Nbatch)
//     this->m_aGrad_W = xt::linalg::dot(DY, xt::transpose(this->m_aCached_X)); // Shape: (Nout, Nin)
//     if(m_bUse_Bias)
...
// xt::xarray<double> FCLayer::backward(xt::xarray<double> DY) {
//     // Calculate gradient of weights with respect to the loss
//     m_aGrad_W = xt::linalg::dot(xt::transpose(m_aCached_X), DY) / DY.shape()[0];

//     // Calculate gradient of bias if it is used
//     if (m_bUse_Bias) {
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY) {
    // Ensure that the input DY has the correct shape
    if (DY.shape().size() != 2 || DY.shape()[1] != m_nNout) {
        throw std::invalid_argument("DY must have shape (batch_size, Nout)");
    }
    ...
    // X: (N, Nin)
    // ΔW: (Nout, Nin)
    xt::xarray<double> DY_T = xt::transpose(DY);
    xt::xarray<double> deltaW = xt::linalg::dot(DY_T, m_aCached_X);
    deltaW /= DY.shape()[0];
    // Accumulate gradients over batches
    m_aGrad_W += deltaW;
    // Compute Δb
    if (m_bUse_Bias) {
        // Sum over the batch dimension (axis=0) to get Δb
        xt::xarray<double> deltaB = xt::sum(DY, {0});

        // Initialize m_aGrad_b to zero if it’s the first time accumulating gradients
        if (m_aGrad_b.size() == 0) {
            m_aGrad_b = xt::zeros_like(deltaB);
        }

        // Divide deltaB by the batch size for averaging, if needed
        deltaB /= DY.shape()[0]; // Assuming DY.shape()[0] is the batch size
    ...
    m_unSample_Counter += DY.shape()[0]; // DY.shape()[0] is the batch size N

    // Return ΔX to propagate back to previous layers
    return dX;
}
