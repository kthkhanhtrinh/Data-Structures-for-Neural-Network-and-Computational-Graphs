  ...
  // TODO YOUR CODE IS HERE
    ...
    for(auto pEntry: list)
    {
        if(keyEQ(pEntry->key, key))
        {
            ...
            table[index].removeItem(pEntry, deleteEntry);
            count--;
            return retValue;
        }
    }
...
DLinkedList<int> xMap<K, V>::clashes() {
  // TODO YOUR CODE IS HERE
    DLinkedList<int> clashes_list;
    
    for(int i = 0; i < capacity; i++)
        clashes_list.add(table[i].size());
...
string xMap<K,V>::toString(string (*key2str)(K&), string (*value2str)(V&)){
    ...
    for (int idx = 0; idx < capacity; idx++) {
        // DLinkedList<Entry*>* list = table[idx];  // Correctly access the list using a pointer
        ...
        for (it = table[idx].begin(); it != table[idx].end(); ++it) {
            Entry* pEntry = *it;  // Get the Entry pointer
            itemos << " (";
        ...
        // Remove the last ';' if there were any entries
